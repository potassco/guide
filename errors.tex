...
If the scripting API~(c.f.~Section~\ref{sec:multi}) is used for grounding,
...
The aggregate occurrence might actually be stratified\comment{there should be some dlv paper we can site here},
in which case the warning can be silenced using option \code{-Wno-nonmonotone-aggregate} (c.f.~Section~\ref{subsec:opt:gringo}).
...
if a nonmonotone aggregate is used recursively.\comment{cite vladimir's paper}
\end{note}
