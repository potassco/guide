...
This warning is emitted if a nonmonotone aggregate is output during grounding:
\comment{MG: 
I suggest using the adjective \emph{recursive} instead of \emph{unstratified}.
The terminology ``recursive aggregate'' is common,
but ``stratified aggregate'' is not.}
\begin{lstlisting}[numbers=none,escapechar=@]
@\textit{Location}@: warning: unstratified nonmonotone aggregate:
  @\textit{Information}@
  (applied translation might produce counter-intuitive results)
\end{lstlisting}
where \codeit{Information} indicates why an aggregate is unstratified
