...
The first integrity constraints ensures that block~\var{X} cannot be moved if there is another block~\var{A} on top of it.
...
The define rule in Line~16 marks the block that has been moved in predicate~\const{moved/1}.
...
Note that the \code{\#show} meta-statement (Line~24--25)
...
is successively replaced with step numbers~\const{0}, \const{1}, \const{2}, and~\const{3}
in the~\const{check} part and step numbers~\const{1}, \const{2}, and~\const{3} in the~\const{transition} part.
While the goal situation checked in the~\const{check} part cannot be
