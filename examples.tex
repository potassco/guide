...
Furthermore, the second integrity constraint excludes all moves where the target block~\var{Y} is blocked because it is below some other block~\var{B}.
...
the constraint only eliminates solutions where the block~\var{X} being moved is different from block~\var{B}, viz., `\code{X != B}'.
