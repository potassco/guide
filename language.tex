...
\com{R2T: please check. This just glosses over saftey. More details?}
...
(note that it does not observe Remark~\ref{rem:aggr-assign}).
...
A weak constraints has a similar form like an integrity constraint (cf. Section \ref{subsec:gringo:normal})
that is associated with a term tuple:
...
We consider the case that it is omitted first.
Whenever the body of a weak constraint is true,
...
In the case that there are different priorities attached to tuples,
we obtain a cost (possibly zero) for each priority.
...
\]
represents the following weak constraints:
\[
...
like incremental solving detailed in Section~\ref{sec:multi}.
...
\tbrc{RK: not complete anymore, if dimacs is listed, opb, wcnf, etc. should be listed too}
%
