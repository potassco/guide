...
any $L_j$ for $1\leq j\leq n$ is a \emph{literal} of the form $A$ or $\code{not}~A$,
...
On the other hand, if only \pred{a} or~\pred{b} is true,
...
Hence, the empty set is the only answer set of the program.
\eexample
\end{example}

Note that, in ASP,
an atom can only be true if it has an (acyclic) derivation from a logic program.
E.g., the log
