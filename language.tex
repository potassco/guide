...
It is extended for \clingo\ with a few directives for multi-shot solving in Section~\ref{subsec:lang:clingo}.\comment{RK: actually not true the language is the same}
...
On the other hand, an integrity constraint is a rule of denial\comment{RK: again},
...
An aggregate is a function on a set of weights\comment{RK: on a set of tuples determined by conditions; I think this difference is important when explaning aggregates}. % ed literals that evaluates to some value. 
...
as detailed in~\cite{siniso02a,gekakasc12a}.\comment{R: I wonder whether this information is important for the guide (and its actually not true, head aggregates are only translated when lparse output is activated)}
...
Hence, this first element is usually specified by means of arithmetic functions (cf.\ Section~\ref{subsec:gringo:arith}).\comment{RK: sounds confusing to me}
%
Finally, 
...
can almost be written in their traditional\comment{R: traditional notation deserves closer explanations right?} notations (without keyword, yet different separators).
