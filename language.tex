...
The general form of an \emph{aggregate atom} is as follows:\comment{T: body aggregate}
%
\begin{lstlisting}[numbers=none,escapechar=?]
?$s_1\prec_1\alpha$?{?$\boldsymbol{t}_1$?:?$\boldsymbol{L_1}$?;?\,\dots\,?;?$\boldsymbol{t}_n$?:?$\boldsymbol{L_n}$?}?$\prec_2 s_2$?
\end{lstlisting}
%
Here, all $\boldsymbol{t}_i$ and $\boldsymbol{L}_i$ are tuples of terms and literals, respectively
...
Note that one of `$s_1\prec_1$' or `$\prec_2 s_2$' can be omitted.
%
For instance, 
...
As indicated by the curly brackets, the literals within aggregates are treated as a set.
Hence, duplicates are not accounted for twice.
%
For instance, the following aggregate atoms express the same:
\begin{lstlisting}[numbers=none,escapechar=@]
#count {a;   not b}       = 1@\textnormal{ and}@ 
#count {a;a; not b;not b} = 1@\textnormal{.}@  
\end{lstlisting}
%
Likewise, the literals of other aggregates are understood as a set.
%
As an example,
consider the program
\begin{lstlisting}[numbers=none]
sum(S) :- S = #sum { C     : cost(_,_,C) }.
mum(S) :- S = #sum { C,X,Y : cost(X,Y,C) }.
\end{lstlisting}
together with the edge costs in Section~\ref{subsec:tsp:instance}.
Here, an atom \lstinline{cost(2,6,4)} means that the arc from node \lstinline{2} to \lstinline{6} has cost \lstinline{4}.
We get an answer set containing \lstinline{sum(10)} and \lstinline{msum(37)}.
Due to the set property, the first rule counts edges with the same cost only once,
while the second accounts for each edge no matter whether they have the same cost or not.
In other words, adding \lstinline{X,Y} to \lstinline{C} enforces a multi-set property;
the same cost can be accounted for several times.
