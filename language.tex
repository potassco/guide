...
\REPc{But unlike (purely) positive literals,
they do not require an (acyclic) derivation from the program;
it is sufficient that they are true in the model at hand.}
{But unlike an atom~$A$,
$\code{not}~\code{not}~A$ is a negative literal that does not
presuppose a derivation of~$A$;
rather, $\code{not}~\code{not}~A$ is assumed to hold if $A$ is true in a model.}
{M: Directly mentioning that double default negated literals are \emph{negative}
    is in my opinion the clearest way to explain the difference to atoms.}
...
% induces no positive cycle between~\pred{a} and~\pred{b} that is subject to minimization.
% Without 
...
that is solved during unification.
...
% E.g., % the tuple 
% \code{(1;(1,))} expands into \code{(1)} and \code{(1,)},
% where \code{(1)} is % not a tuple here but 
% equal to the integer~\code{1}.}
...
instead of disjunction, unless the latter is required for complexity reasons.
This also means that conditions should
...
Aggregates are expressive % one of the most useful 
modeling constructs % since they allow for forming values from groups of selected items.
that map groups of terms to values.
Together with comparisons
they allow for formulating conditions over these terms.
...
Within both aggregate atoms, % symbolic 
...
\end{lstlisting}
%
Here, all $L_i$ are % symbolic 
literals as introduced in Section~\ref{subsec:gringo:normal}, while all other entities are as described above.
The second colon in
...
whenever the distinguished % head 
literals $L_i$ are different.
%
...
% Same head 
Recurrences of
literals yield identical terms, as we see next.
The rule
\begin{lstlisting}[numbers=none]
{ a; a }.
\end{lstlisting}
...
as put forward in the traditional \lparse\ grounder~\cite{lparseManual}.
\end{note}
...
(see Remark~\ref{note:domain})  % (see Page~\pageref{pg:domain}) 
or built-ins,
...
% More importantly,
The role of aggregates here % in the last two lines 
...
which do not follow the recommendation in Remark~\ref{rem:aggr-assign}, % these two lines 
...
\marginlabel{%
  ...
  \code{\mbox{~}gringo \attach{examples/aggr.lp}{aggr.lp} | \textbackslash\\
        \mbox{~}clasp 0}}
%
In fact, running \clingo\ or \clasp\ shows % the student 
that a unique
...
\]
% Note that each $\boldsymbol{t}_i$ can be preceded by an optional prioritized term $w_i$@$p_i$.
%
...
A \code{Fun} object has an additional function \code{args()} returning the table % of arguments and 
or list of arguments, respectively, in Lua and Python.
% In Python, 
Moreover,
tuples are mapped to Python's tuple data structure and 
% And because there are no dedicated tuples in Lua, tuple terms are mapped 
to \code{Fun} objects
...
% there is the \code{Id(name)} constructor, which 
The \code{Id(name)} constructor
simply takes the string representation of a constant as argument,
% Similarly, for functions,
% there is 
and the \code{Fun(name,args)} constructor additionally takes
% taking a string representation of the name of the function
% together with 
a sequence or table of terms representing the arguments of a function.
% as arguments.
Furthermore, % there is 
the global function \code{Tuple(args)} % , which
is a shortcut for \code{Fun("",args)}
to create an object representing a tuple in Lua.
The remaining terms,
...
the \code{>} relation 
% here because it cannot compare objects of different types,
cannot be applied directly because objects can be of different types,
...
That is, when a function is called multiple times with the same arguments
during grounding,
then it should % always 
return the same values.
Adding % non-deterministic 
functions that do not comply with this assumption can lead to undesired results.
\item If an error occurs during the evaluation of an external function, % then 
a warning is printed and the current instance of a rule or condition is dropped.
\comment{M2R: Please check the reformulation of the previous sentence. Thanks!}
% substitution is thrown away.
For example, this happens when % calling 
the~\code{gcd} function from % in 
Example~\ref{ex:gcd} is applied to % with 
...
% Sometimes, one may be interested only in
...
% mark atoms as external.
% This means that those atoms are not 
...
The main use case 
is to implement extensions to plain ASP solving,
...
\noindent
%
where $p$ is the program part name and the parameters $s_i$ are % symbolic 
constants.
If $n$ is zero, then the parentheses can be omitted.
All rules, external statements, and show statements for terms
% following such a directive
up to the next \code{\#program} statement or the end of the file belong to the program part $p/n$.
Rules that are not subject to % a program part 
any such directive are included in the \code{base}/$0$ part.
...
Occurrences of % symbolic 
constants that are parameters of a part
...
% Note the call to solve in Line 10. This 
...
For single-shot solving, program parts are not needed, % should not be used.
but the feature is % merely 
listed for completeness here.
\end{note}

\paragraph{Include Statements}
Include statements allow for including files from within another file.
They have the form % following form:
%
\par
\medskip
\begin{tabular}{l}
...
\end{lstlisting}
%
% Because files are included from the current working directory,
...
Solver \clasp~\cite{gekanesc07b} (or `\clingo\code{ --mode=clasp}')
% can be used to solve ASP, SAT, Maximum Satisfiability (MaxSAT; \cite{liman09a}),
% and Pseudo-Boolean (PB; \cite{rouman09a}) constraint satisfaction/optimization problems.
% To this end, \clasp\
accepts logic programs in \smodels\ format~\cite{lparseManual},
