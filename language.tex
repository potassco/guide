...
They are satisfied whenever their positive counterparts are\REPc{.
But unlike (purely) positive literals,
they do not require an (acyclic) derivation from the program.}%
{, i.e., $\code{not}~\code{not}~A$ holds as soon as $A$ is considered true.}%
{MG: What about this?}
...
\end{lstlisting}
%
This program has the same answer sets as the one in Example~\ref{ex:as:one}
as well as the additional answer set containing both \pred{a} and \pred{b}.
This is because neither `\code{not not a}' nor `\code{not not b}' require an acyclic derivation from the program.
