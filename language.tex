...
It is extended by \iclingo\ with a few incremental directives described in Section~\ref{subsec:lang:clingo}.
...
In general, the use of disjunction may % however
increase
...
\marginlabel{%
  ...
  \code{\mbox{~}clingo -t \attach{examples/arithf.lp}{arithf.lp}}\\
  or alternatively:\\
...
\marginlabel{%
  ...
  can be inspected by invoking:\\
  \code{\mbox{~}clingo -t \attach{examples/arithc.lp}{arithc.lp}}
  or alternatively:\\
...
\marginlabel{%
  ...
  \code{\mbox{~}clingo -t \attach{examples/symbc.lp}{symbc.lp}}\\
  or alternatively:\\
...
\marginlabel{%
  ...
  can be inspected by invoking:\\
  \code{\mbox{~}clingo -t \attach{examples/assign.lp}{assign.lp}}\\
  or alternatively:\\
...
\marginlabel{%
  ...
  can be inspected by invoking:\\
  \code{\mbox{~}clingo -t \attach{examples/unify.lp}{unify.lp}}\\
  or alternatively:\\
...
\marginlabel{%
  ...
  can be inspected by invoking:\\
  \code{\mbox{~}clingo -t \attach{examples/int.lp}{int.lp}}\\
  or alternatively:\\
...
\marginlabel{%
  ...
  \code{\mbox{~}clingo -t \attach{examples/cond.lp}{cond.lp}}\\
  or alternatively:\\
...
In fact, the literals on the right-hand side of a condition are connected conjunctively, that is,
...
\begin{enumerate}
\item 
All predicates of atoms on the right-hand side of a condition
...
\marginlabel{%
  ...
  \code{\mbox{~}clingo -t \attach{examples/pool.lp}{pool.lp}}\\
  or alternatively:\\
...
\marginlabel{%
  ...
  can be inspected by invoking:\\
  \code{\mbox{~}clingo -t \attach{examples/pools.lp}{pools.lp}}\\
  or alternatively:\\
  \code{\mbox{~}gringo -t \attach{examples/pools.lp}{pools.lp}}}
%
...
An aggregate is a function from a multiset of weights. % ed literals that evaluates to some value. 
...
The general form of an \emph{aggregate atom} is as follows:
%
\begin{lstlisting}[numbers=none,escapechar=?]
?$l\;\mathsf{op}$\:?[?\:$L_1$\,?=?\,$w_1$?,?\,\dots?,?\:$L_n$\,?=?\,$w_n$\:?]?\:$u$\,\textnormal{.}?
\end{lstlisting}
%
Here, all $L_i$ are literals (as introduced in Section~\ref{subsec:gringo:normal})
associated with weights~$w_i$, specified by means of arithmetic functions
(cf.\ Section~\ref{subsec:gringo:arith}).
Furthermore, $\mathsf{op}$ is the name of some function that is to be
applied to the weights of literals that hold.
Finally, $l$ and~$u$ are arithmetic functions that determine a lower and an upper bound
for the result of applying $\mathsf{op}$.
That is, an aggregate atom holds if the result of $\mathsf{op}$
applied to the weights of literals that hold
is in-between $l$ and~$u$ (both inclusive).
...
\code{\#avg}
\index{Aggregates!Average, \code{\#avg}}%
(the average of weights%
\footnote{
Note that an aggregate atom using \code{\#avg}
is defined to hold regardless of its bounds
if none of its literals holds, i.e.,
if the average is taken over the empty set of weights.}%
).
Aggregate atoms, as described above, are obtained by writing
either \code{\#sum}, \code{\#min}, \code{\#max}, or \code{\#avg} for $\mathsf{op}$.
In addition to them,
there are three more aggregates
whose atoms obey a slightly different syntax:
% that are syntactically different.
% The first is the \code{\#count} aggregate:
%
\begin{lstlisting}[numbers=none,escapechar=?]
?$l\;$?#count?\:?{?\:$L_1$?,?\,\dots?,?\:$L_n$\:?}?\:$u$\,\textnormal{,}?
#even?\:?{?\:$L_1$?,?\,\dots?,?\:$L_n$\:?}?\textnormal{, and}?
#odd?\:?{?\:$L_1$?,?\,\dots?,?\:$L_n$\:?}?\textnormal{.}?
\end{lstlisting}
...
is similar to \code{\#sum} with all literals' weights being~\const{1}.
However, as indicated by curly brackets,
the literals of \code{\#count} are understood as a set,
so that duplicates are not counted twice.
For instance, note that the following aggregate atoms express the same:
\begin{lstlisting}[numbers=none,escapechar=@]
1 #sum   [a=1, not b=1]     1@\textnormal{,}@ 
...
% \end{align*}%
Likewise, the literals of parity aggregates \code{\#even} and \code{\#odd},
whose atoms hold if the number of literals that hold is even or odd, respectively,
are understood as a set.
Also note that parities are not compared to any bounds.

There are a number of shorthands that can be used in the
syntactic representation of aggregates.
...
\end{lstlisting}
Likewise, we can optionally omit \code{\#count} in front of curly brackets:
\begin{lstlisting}[numbers=none,escapechar=@]
1 #count {a, not b} 1 @\textnormal{and}@ 
...
For the aggregates \code{\#sum}, \code{\#min}, \code{\#max}, and \code{\#avg},
...
\code{\pred{max}(\const{3})}, and \code{\pred{cnt}(\const{1})} are
...
\begin{lstlisting}[numbers=none]
bot :- #min [a=#infimum] -1000.
top :- 1000 #max [a=#supremum].
\end{lstlisting}
...
\subsection{Input Language of \iclingo}\label{subsec:lang:clingo}
