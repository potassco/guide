...
and disjunctively in the body of a rule, respectively.
...
\comment{T2R: Reference to ``no one-elementary tuples''}
...
\eexample
\end{example}
\comment{T2R: Example/Note on difference to intervals}
...
\REWc{very similar to rules}{T:???}.
The following example, making exhaustive use of aggregates,
demonstrates a lot of different features
(note that it \REPc{does not observe}{ignores}{T2R:?} Remark~\ref{rem:aggr-assign}).
...
The form of weak constraints is similar to integrity constraint (cf.~Section \ref{subsec:gringo:normal})
...
the \code{\#show} \REP{declarative}{directive} can be used.
...
This feature is especially handy when applying meta-programming techniques (cf. Section~\ref{section-yet-to-come})\comment{RK: todo add ref}
...
Via the \code{\#const} \REP{declarative}{directive},
...
like \REP{incremental}{multi-shot solving} solving detailed in Section~\ref{sec:multi}.
...
the part \REP{beginning with}{following} the~\code{:} is a \INS{tuple of} condition\INS{s}.
...
printing the symbol table of the lparse format~\cite{lparseManual}.
\eexample
\end{example}
...
\noindent
%
where {\itshape file} is a path to an encoding file.
...
\tbrc{RK: not complete anymore, if dimacs is listed, opb, wcnf, etc. should be listed too\par T: We need Benjamin!}
%
