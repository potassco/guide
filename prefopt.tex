...
implementing the preference specificiation; see below).
...
\comment{T: Shall we talk about different negations?}
...
\code{t} is a \REWc{ground}{T2J: Why?} term providing the preference type, 
...
\begin{example}\label{asprin:example3}\comment{T2J: Removed `hot'. Changed to -bunji (or take `not bunji')? What do you think?\par PROPAGATE CHANGE!!}
...
For example, the type \code{subset} maps $E=\{e_1\code{;}\ldots\code{;}e_n\}$ into
\[
...
#preference(s,t){?${e}_1$?;?$\dots$?;?${e}_n$?} : ?$B$?.
\end{lstlisting}
Acoordingly, the weighted formula is translated into a rule of the form
\begin{lstlisting}[numbers=none,escapechar=?]
...
by using function symbols \REWc{$\code{\_neg/1}$}{T2R: Now we use '-', or??\par also we need: \textit{not}\par PROPAGE CHANGE}, $\code{\_and/2}$, and $\code{\_or/2}$ in prefix notation.
For example, 
...
\begin{example}
\comment{JR: This is a ground preference specification, maybe we could add variables.\par T: See above ;)}
...
it is included in a preference-type independant program named \code{preference}.
...
Thus, \code{\_error(X)} has to be true whenever the preference statement is not admissible.
In this case, \asprin\ exits and prints the error message \code{X}.\footnote{%
