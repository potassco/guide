...
\marginlabel{%
  To inspect the output, invoke:\\
  \code{\mbox{~}asprin \attach{examples/base.lp}{base.lp} \
  \attach{examples/cpreference.lp}{cpreference.lp} \ 
  asprin.lib 0}}
%
...
while optimizing \code{p9} we get only $X_2$.
\end{example}
...
%
\lstinputlisting[basicstyle=\small\ttfamily,numbers=none,firstline=20,lastline=33]{examples/subset.lp}
%
...
For implementing composite preference types, we also provide a definition for predicate \code{better/1},
but now the implementation relies on predicates 
...
%
The definition uses predicates \code{better/1} and \code{bettereq/1}, 
that must be correctly defined by the corresponding preference types of the named preference statements. 
For instance, for \code{subset} we already have the definition of \code{better/1} from Example \ref{asprin:subset_imp}, 
so we could simply extend program \code{\#preference(subset)} with the following rule:
%
\lstinputlisting[basicstyle=\small\ttfamily,numbers=none,firstline=6,lastline=7]{examples/subset.lp}
%
As another example, \code{less(cardinality)} can be implemented via the following program:
%
\lstinputlisting[basicstyle=\small\ttfamily,numbers=none,firstline=1,lastline=9]{examples/less-cardinality.lp}
%
Putting all this together, we can compute the optimal answer sets of the program:
%
\lstinputlisting[basicstyle=\small\ttfamily,numbers=none]{examples/pareto_preference.lp}
%
with the following command:
\begin{lstlisting}[numbers=none,escapechar=\%]
asprin %\attach{examples/base.lp}{base.lp}% %\attach{examples/preference1.lp}{preference1.lp}% %\attach{examples/subset.lp}{subset.lp}% %\attach{examples/basic.lp}{basic.lp}% 0
\end{lstlisting}
As in Example \ref{asprin:composite}, we obtain $X_1$, $X_2$ and $X_3$.
\end{example}

\begin{note}
The correctness of the implementation of a composite preference type relies on the 
implementations of the preference types to which it relates via naming atoms.
For the preference types implemented in the \asprin\ library, 
we provide definitions of predicates 
\code{better/1}, \code{bettereq/1}, \code{eq}, \code{worseeq/1}, and \code{worse/1}, 
representing the relations $\succ$, $\succeq$, $=$, $\preceq$, and $\prec$, respectively. 
Note that other predicates could be defined.
\end{note}
