...
\code{$E$} where each $e\in E$ has the form:
\begin{lstlisting}[numbers=none,escapechar=?]
?${w}$?,?$\boldsymbol{t}$?::?$F$?
\end{lstlisting}
...
Then, $E$ is mapped to the preference relation,
where $X \succ Y$ holds if 
...
name(s)
\end{lstlisting}
For preference type \code{neg}, the sets must be singleton.
And for \code{lexico}, each naming atom is attached to a weight\comment{T2J: tuple??? just to make sure} $\code{$w$}$: 
\begin{lstlisting}[numbers=none,escapechar=?]
?${w}$?::name(s)
\end{lstlisting}
...
\comc{Implementing composite preferences.}{T2R: ??}
