...
\comment{T: Shall we talk about different negations?\par J: I would not mess with it... \par J: Do we allow to nest negation? If not, we should mention it. }
...
\code{t} is a \REWc{ground}{T2J: Why? \par
J: This is for implementation reasons, and may be changed.
   When parsing, asprin only loads (1) the preference programs whose ``names'' 
   appear in some preference statement and (2) the preference programs with no types. 
   This is good for efficiency, so that we can add a big library and ony load what is needed.
   To implement this, we require that these term types are ground, and in this way we 
   can compare (before grounding) with  the preference program names.
} 
...
\begin{example}\label{asprin:example3}\comment{T2J: Removed `hot'. Changed to -bunji (or take `not bunji')? What do you think?\par
J: I prefer to use ``not'', so we do not mess with strong negation. 
\par
J: With \code{temp(C), C > 25} the element was not safe (see above), 
so I moved back to \code{hot} and added a variable at \code{costs}. Take it as a suggestion :)
}
...
by using function symbols \REWc{$\code{\_neg/1}$}{T2R: Now we use '-', or??
\par J: No, here we can use the same, given that strong negation is handled normally by gringo.
\par also we need: \textit{not}\par PROPAGE CHANGE \par J: I think what we have is enough now :)}, $\code{\_and/2}$, and $\code{\_or/2}$ in prefix notation.
For example, 
the formula \code{(not a(X) | b(X)) \& c(X)} is translated into \code{\_and(c(X),\_or(\_neg(a(X)),b(X)))}. 
%
...
\begin{example}
\comment{JR: This is a ground preference specification, maybe we could add variables.\par T: See above ;) \par J: I will change this later once we agree on the example.}
...
is limited to that of stratified logic programs\comment{JR: Add cite}.
