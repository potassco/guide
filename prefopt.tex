...
refer to the preference associated with statement \code{s} (see below).
%
...
`\code{X::name(p(X))}'. 
If $F_1$, \ldots, $F_n$ are weighted formulas,
\begin{lstlisting}[numbers=none,escapechar=?]
{?$F_1;\dots;F_m$?}
\end{lstlisting}
...
\comment{T2J: Only each variable must be bound by a domain predicate, or? 
\par JR: No, to fit nicely with the theory, all atoms in the body must be over domain predicates, 
         so that after grounding they ``disappear'' and all stable models share the same preference statements }
%
We may drop `\code{>}' if $m=1$, 
and `\code{||$\,F$}' and `\code{:$\,B$}' if $F$ and $B$ are empty, respectively.
%
...
According to that, the literals of $B$ must belong to domain predicates or built-ins of the accompanying logic program.
This ensures that $B$ can be fully evaluated by the grounder.
\comment{JR: Not sure here, maybe something more could be added. 
             Something like: ``so that all stable models share the same ground preference statements'', 
             but on the other hand I think this is something too complicated at this point of the presentation.}
Note that to express conditions on preferences one can use the conditional expressions `\code{||$\,F$}'.
\end{note}
...
each $e_j$ is a preference element, 
and the rule body $B$ has the same form as above.

\begin{note}
The body $B$ of a preference statement is used to instantiate the variables of $s$, $t$ and \emph{every} $e_i$.
...
\end{lstlisting}
stands for the two ground preference statements:
\comment{JR: This ``stands'' is a bit fuzzy, but maybe that's the right thing at this point.}
\begin{lstlisting}[numbers=none]
...
\emph{optimization directives}
\begin{lstlisting}[numbers=none,escapechar=?]
#optimize(s) : ?$B$?.
\end{lstlisting}
where $B$ is as before, 
telling \asprin\ to restrict its reasoning mode to the preference relation declared by \code{s}.
% 

A \emph{preference specification} is a set of preference statements and optimization directives.
A \emph{ground} preference specification is \emph{valid} if the naming dependencies are acyclic and closed, 
and there is a single optimization directive 
(see \cite{brderosc15a} for details).
A \emph{non ground} preference specification is \emph{valid}
...
\comment{JR: Maybe add something here?}
\begin{example}
