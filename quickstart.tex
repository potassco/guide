...
%that correspond to the pegs and disks in the puzzle.
...
with the requirement that the goal ought to be achieved with\comment{BK: within?}~\const{15} moves.
...
\comment{RK: Here cardinality constraint is used but the notion is only given in a remark at the end of the Shortcuts paragraph. Maybe we should not call this paragraph Cardinality Constraints?}
...
that you will not see the delay.
