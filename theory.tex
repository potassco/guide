...
and whenever \code{a} holds, the linear equation $\code{x}-\code{y}\leq -3$ must also hold.
...
while in principle there are infinitely many valid assignments for $\code{x}-\code{y}\leq -3$.
...
% --------------------------------------------------------------------------------
%
To see a more involved example, we consider the flow shop problem,
...
This option changes the logical connection of the truth value of the difference constraint atom and the difference constraint it represents.
The connections are as follows:
\begin{align*}
 \text{non-strict :}&\code{\&diff\text{ }\{$x$-$y$\}\text{ }<=\text{ }$k$} \rightarrow x-y\leq k\\
 \text{strict :}&\code{\&diff\text{ }\{$x$-$y$\}\text{ }<=\text{ }$k$} \leftrightarrow x-y\leq k
\end{align*}
...
the difference constraint $y-x\leq 2$ has to hold, which is the negation of $\code{x}-\code{y} \leq -3$.
...
Only the answer set with \code{a} remains and the assignment of \code{y} changed to 4 
...
Since there does not exist a valid assignment fulfilling those two constraints, it cannot belong to an answer set.

\end{example}

Let us now consider the usefulness of strict and non-strict semantics in different scenarios.
As a rule of thump, using difference constraint atoms in the head of rules fits the non-strict semantics 
...
However, switching to non-strict semantics produces the following result:
\marginlabel{%
  To inspect the output, invoke:\\
  \code{\mbox{~}clingo-dl \textbackslash\\ 
