...
%
The following description is conform with the \clingoM{LP} system available at \url{http:github.com/potassco/clingoLP}.
...
\marginlabel{
  To inspect the output, invoke:\\
  \code{\mbox{~}clingo clingoLP.py \textbackslash\\ 
        \mbox{~~~}\attach{examples/lp1.lp}{lp1.lp}   0}\\
}
\begin{lstlisting}[numbers=none]
...
LP Solver output
...
which is indicated by \code{None} below \code{LP Solver output}. 
%
...
\marginlabel{
  To inspect the output, invoke:\\
  \code{\mbox{~}clingo clingoLP.py \textbackslash\\ 
        \mbox{~~~}\attach{examples/lp2.lp}{lp2.lp}   0}\\
}
\begin{lstlisting}[numbers=none]
...
LP Solver output
...
\marginlabel{
  To inspect the output, invoke:\\
  \code{\mbox{~}clingo clingoLP.py \textbackslash\\ 
        \mbox{~~~}\attach{examples/lp2.lp}{lp2.lp}  \textbackslash\\ 
        \mbox{~~~}-c accuracy=3  0}\\
}
To print the assignment with three digits after the decimal dot run the same program with the additional parameter \code{-c accuracy=3}.
%
...
then this can be easily done by adding \code{-c ilp=1}. 
%
