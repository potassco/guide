...
the automatic \REPc{algorithm}{configuration}{T2M: I guess that's better in the context of the guide, or?} selection solver \claspfolio~\cite{holisc14a} and 
...
%
\begin{description}
  \item[\code{--instances,-I}] \comment{T2M: a directory containing??}a set of grounded instances on which the performance of \clasp\ will be optimized.
...
\comment{T2M: Why use piclasp as a script and not standalone??}
\begin{lstlisting}[numbers=none]
