...
can be found.
In view of this,
it is convenient to encode % reduce 
the particular problem in % to
a declarative problem solving paradigm like ASP,
where efficient off-the-shelf tools like \gringo\ and \clasp\
are available. % ready to solve the problem reasonably well.
...
\end{lstlisting}
%
Note that we have omitted the atoms over % six instances of
\pred{node}/$1$ and % the 17 instances of 
...
In addition, facts over % predicate 
\pred{cost}/$3$ are used to define edge costs:
%
\lstinputlisting{examples/costs.lp}
%
Figure~\ref{fig:costs} shows the % (directed) 
...
% Note that this definition admits 
...
% Boolean Satisfiability~\cite{SATHandbook} or
% Constraint Programming~\cite{CPHandbook}.
% In our present problem, this feature makes
...
\marginlabel{%
  ...
  \code{\mbox{~}gringo \attach{examples/ham.lp}{ham.lp} \textbackslash\\
        \mbox{~}\attach{examples/graph.lp}{graph.lp} | % \rlap{\:\textbackslash}\\\mbox{~}
        clasp 0}
}
Finally, the additional display part in Line~10 states that answer
sets should be projected to instances of \pred{cycle}/$2$, as only they % describe 
...
After grounding, the % minimization
\code{\#minimize} statement
...
and the \code{check(t)} part % is instantiated 
...
%
% In the first line we enable 
The first line enables \clingo's incremental computation mode.
%
Next, % in 
the \const{base} part in Line~3--7 % we define
defines blocks and the constant \const{table}
as instances of predicate \pred{location}/$1$.
Moreover, we use instances of \pred{init}/$1$
to initialize % predicate
\pred{holds}/$2$ for the initial state at step~\const{0}, thus
...
The \const{transition} subprogram in Line~9--18 declares constant~\code{t} as a placeholder for step numbers in the program part below.
...
Furthermore, the second integrity constraint excludes all moves where the target block~\var{Y} is occupied % because it is below 
by some other block~\var{B}.
...
% the constraint 
it only eliminates solutions where the block~\var{X} being moved is different from% block
~\var{B}, viz., `\code{B~!=~X}'.
The % define
rule in Line~16 marks the block that is moved via predicate \const{moved/1}.
Finally, the % define
rule in Line~17 propagates a move to the state at step~\code{t},
while the rule in Line~18 states
that a block~\var{X} stays at a location~\var{Z} if it is not moved.

The subprogram \code{check} in Line~20--22 % allows for specifying 
specifies goal conditions % constraints
to be checked for each state.
% We check the goal condition in the following test part.
Note the use of atom \code{query(t)}.
This atom, provided in incremental mode, % to post goal queries
...
Note that the \code{\#show} meta-statement in Line~25
does not belong to % is not part of
any program part
...
heads of rules in Line~11--18. % , Line~16, and~Line~17--18, respectively.
...
where, starting from the \const{base} and the \const{check} part in which% constant
~\const{t} is replaced with% step number
~\const{0}, 
the constant~\const{t} is successively replaced with step numbers~\const{1}, \const{2}, and~\const{3} in the~\const{transition} and \const{check} parts.
While the goal % situation tested 
conditions in the \const{check} part cannot be fulfilled in steps~\const{0}, \const{1}, and~\const{2},
\clingo\ stops its incremental computation after finding an answer set in step~\const{3}.
The scheme of iterating steps until finding % at least one 
