...
allowing to modify the heuristic of the solver from within a logic program or from the command line.
...
\comment{JR: This should be made coherent with the Incremental Solving Section, 
    ...
    \par TS: Or even move into this section\dots?}
...
At the start of the search, the goal's \code{on} atoms are true at the last time instant.
With this rule, the solver decides on a \code{move} atom that makes one of them true.
Then, some \code{on} atoms are propagated to the previous time instant,
...
First, the solver decides on one of the goal's \code{on} atoms at the last but one time instant,
...
to the atoms that appear in optimization statements, 
and with option \code{--dom-mod=1,20},
we apply modifier \code{level} with value \code{1}
to both the atoms that appear in disjunctions as well as to shown atoms.
...
On the other hand, they allow to directly refer to structural components of the program 
(strongly connected components, head cycle free components, etc.)
and they do not require any grounding, 
...
\par   TS: Indeed that may be worth a little example :)}
