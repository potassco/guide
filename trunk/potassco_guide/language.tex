...
Rules\comment{RK: what to use as a seperator of body elements?} of the following forms are admitted in a 
...
\index{Logic Programs!Head Literals}

The input language of \gringo\ also supports doubly default negated literals,
written $\code{not}~\code{not}~A$.
They are satisfied whenever their positive counter-parts are satisfied. 
But unlike with positive literals,
such literals do not span positive cycles subject to minimization.
...
This program has the same stable models as the program in Example \ref{ex:as:one} 
as well as the additional stable model where both \pred{a} and \pred{b} are true
because there is no positive cycle subject to minimization between~\pred{a} and~\pred{b}.
\eexample
\end{example}

Then there are literals in the head of rules.
...
Hence, the answer set where tweety does not fly does not contain any atoms over predicate \pred{flies}/$1$.
\eexample
\end{example}

\begin{Note}
  Note that negated head literals are also supported in disjunctions.
...
does not count as a positive occurrence in the sense of safeness\comment{RK: safeness vs. safety; X in X+1 is actually safe} (cf.\ Page~\pageref{pg:safe}).
...
The following built-in predicates\comment{RK: safeness? these can be used like literals now}, which are evaluated during grounding, 
...
Also note that comparison predicates must not be preceded by \code{not}:\comment{RK: this is possible in the latest gringo trunk}
...
\com{Roland}
\com{RK: asignment := is gone, lhs and rhs does not matter}
...
Also observe the different usage and role of built-in comparison predicate~\code{==}.\comment{in principle == is completely unnessecary in view of =}
\eexample
\end{example}
...
\com{RK: Here things get ugly because we have to explain the , versus ; thing, also there are less restrictions on conditionals now}

Conditions allow for instantiating variables to
collections of terms\comment{RK: strange in the context of a conditional literal} within a single rule.
...
\comment{R: I thought there ought to be no formal details in the guide; this remark does not fit in with the style of the rest of the document}
...
An aggregate is a function on a set of tuples that are noprmally subject to conditions. 
...
Simplifying shortcuts providing some backward compatibility with previous versions of \gringo\ are given below.
Unlike with body aggregates, only singular head aggregates are supported.

Aggregate atoms in the head can be understood as a combination of unrestricted choices with body aggregates enforcing the constraint of the original aggregate.
...
so-called 
``weight constraints'' and ``cardinality constraints''~\cite{siniso02a}
