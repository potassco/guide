...
Solver \clasp~\cite{gekanesc07b} can be run as a solver for ASP, SAT, Maximum Satisfiability (MaxSAT; \cite{liman09a}),
and Pseudo-Boolean (PB; \cite{rouman09a}) constraint satisfaction/optimization.
...
and DIMACS-wcnf format\footnote{\url{http://www.maxsat.udl.cat/12/requirements/index.html}},
and PB problems in OPB/WBO format\footnote{\url{http://www.cril.univ-artois.fr/PB12/format.pdf}}.
...
Finally, \clasp\ may be used as a (C++) library, as done within \clingo.
...
\footnote{%
  Option \code{--file} or its short form \code{-f} can be used to
  distinguish a file name from a number
  (cf.\ Section~\ref{subsec:opt:clasp}).}
\begin{lstlisting}[numbers=none]
