...
\com{R: I want to replace the obstruse example below with something simpler}
\begin{example}\label{ex:intsimple}
Consider the following program:
\begin{lstlisting}[numbers=none]
grid(1..3,1..3).
\end{lstlisting}
Since the intervals in this rule all occur in the rule head,
...
Similarly, intervals can be used in the body.
...
\eexample
\end{example}
The grounding of this rule results in the same set of facts as the previous rule.
...
in the sense of safety (cf.\ Page~\pageref{pg:safe}).
Variables occurring in atoms not subject to any condition are \emph{global}.\comment{T: Reference point, eg rule\dots? R: Should not be necessary, because uniquely determined by ``not subject to a condition''.} 
...
\com{R: I think we need a less scary example to open this section to not put the reader off.}
...
is a shortcut for\comment{RK: this is not correct because \code{1~\#count~\{1:a;~2:a\}~1} is not a shortcut for \code{1~\{a;~a\}~1}}
%
\begin{lstlisting}[numbers=none,escapechar=?]
...
By allowing the omission \const{\#count},
...
That is, an interval gives rise to multiple aggregate elements connected via~``\code{;}''
