...
\com{RK: From now on I will pretend that gringo only outputs a semicolon to terminate a conditional literal}
...
Note that ``\code{,}'' is used both to separate literals in rule bodies and conditions.
To resolve this ambiguity, 
a condition has to be terminated with ``\code{;}'' if further body literals follow.

\begin{example}\label{ex:sort}
The following program uses a literal with a composite condition in the middle of the rule. 
Note the ``\code{;}'' after the condition:
%
\lstinputlisting{examples/sort.lp}
%
The literal with condition in the second rule evaluates to false
...
If on the other hand, ~\code{X} and~\code{Z} succeed each other,
the condition is false for all elements~\code{Z}.
This means that the literal with conditon stands for an empty conjunction, which is true:%
\marginlabel{%
  The reader can reproduce these ground rules by invoking:\\
...
\end{lstlisting}
We obtain an answer set where the elements of \code{set}/$1$ are ordered.
\eexample
\end{example}
...
\item
During grounding,
global variables take precedence over non-global ones, i.e.,
