...
\lstinputlisting[language={[5.1]Lua}]{examples/gcd-lua.lp}
\end{minipage}
\begin{minipage}[t]{0.51\textwidth}
\lstinputlisting[language=python]{examples/gcd-py.lp}
\end{minipage}\\
...
%
This function is called in Line~3 and the result stored in predicate \code{gcd}/$3$.
...
\lstinputlisting[language={[5.1]Lua}]{examples/rng-lua.lp}
\end{minipage}
\begin{minipage}[t]{0.49\textwidth}
\lstinputlisting[language=python]{examples/rng-py.lp}
\end{minipage}\\
...
%
This function is called in Line~3 and the result stored in predicate \code{rng}/$3$.
...
\com{RK: split the examples,
Outline:
term inspection/construction (give table of available functions, possible examples: packing/unpacking of tuples, string concatenation, mention reference doc of module here),
inbuilt comparison function (for min/max function),
stress deterministic behavior of functions (uid example),
errors in functions (eg. calling gcd with non-numeric arguments)}

\begin{itemize}
  \item Numbers - mapped to lua numbers, to python integers
  \item Strings - mapped to lua and python strings
  \item Id      - mapped to Id objects that have a name() method to obtain its name
  \item Tuple (lua only) - tuples are mapped to python tuples, in lua there is no such thing as a tuple so there is a Tuple object having method args() to obtain a table of arguments of the tuple
  \item Fun      - mapped to Fun objects having method args() and name()
  \item Sup, Inf - mapped to Sup and Id constants (is or == can be used)
  \item 
\end{itemize}
