...
a conditional literal has to be terminated with ``\code{;}'' if further body literals follow.

\begin{example}\label{ex:sort}
The following program uses a conditional literal with a composite condition in the middle of the rule. 
Note the ``\code{;}'' after the conditional literal:
%
\lstinputlisting{examples/sort.lp}
%
The conditional literal in the second rule evaluates to false
whenever there is an element~\code{Y} between~\code{X} and~\code{Y}.
Hence, all rules instantiations where~\code{X} and~\code{Y} are not direct successors are discarded
