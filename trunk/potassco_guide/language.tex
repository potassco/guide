...
\comment{JR: For me the use of ``models'' and ``answer sets'' here is a bit confusing.
             Are models answer sets, or classical models? %JR
             Maybe we could always talk about answer sets (?). }%JR
...
\comment{JR: I got lost :( 
             What about something like what is written below? %JR
             ``They are satisfied whenever their positive counterparts are, %JR 
             but they do not have to be finitely derivable. And then move to the example.''}%JR
...
\comment{JR: For me this sounds too complicated: an induced positive cycle subject to minimization.}
...
\comment{JR: Style: maybe move the ``are given'' to the end of the sentence?} 
...
\comment{JR: Style: Add comma? or: 
             ``In the head of the rule an interval is expanded conjunctively, %JR
             while in the body it is expanded disjunctively'' } %JR
...
but with the difference that unlike intervals
\comment{JR: Style: Comma? }
pools have a fixed size:%
\marginlabel{%
...
\comment{JR: ``Because all the pools in this rule occur in the head''?}
...
\comment{JR: this ``tuple'' strikes me a bit, then I have the doubt about whether they should go in parentheses}
...
\comment{JR: Style: ``they allow to form values''?}
...
thus,
\comment{JR: Style: I would get rid of this comma}
...
\comment{JR: the meaning of ``usually representing arithmetic functions'' is not clear to me}
...
respectively.
\comment{JR: ``respectively'' should go into the parentheses, oder?}
...
\comment{JR: Maybe the fact that the first element is the weight could be stressed more,
             but I am not sure about how to do it.}%JR
...
\end{lstlisting}
Both aggregate atoms can be used like any other atom, possibly preceded by negation, in a
\comment{JR: Style: change ``a'' for ``the''?}
body of a rule.
...
\comment{JR: This is a bit confusing to me. Literals of aggregates are understood as sets, 
             but here I think the point is more that term tuples whose conditions are satisfied are understood as sets.}%JR
%
...
\comment{JR: Style: Delete this respectively?}
...
\comment{JR: I would explain a bit what is a ``singular head aggregate''. 
             ...
             and I think that's all said so far about it.}%JR
\begin{note}
...
\comment{JR: Style: I would write here the symbols $s_1\prec_1$ and $\prec_2 s_2$ to remember what a guard is.}
...
\comment{JR: Style: this ``should be enrolled'' sounded a bit strange to me, but maybe it's me ;)}
%
...
\comment{JR: Style: sounds strange (to me?)}
...
%
\comment{JR: Style: This change of numbers lost me, although I recovered ;)}
As Line~15 shows,
...
it contributes its term tuple (as with aggregates
\comment{JR: Style: Comma?}
duplicates are ignored) to a cost function.
...
\comment{JR: No weights here? $w_1$@$p_1$,$\boldsymbol{t}_1$:$\boldsymbol{L}_1$}
\[
