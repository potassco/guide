...
\subsubsection{Double Default Negation and Literals in the Head of Rules}\label{subsec:gringo:double}
...
The input language of \gringo\ also supports doubly negated literals, written $\code{not}~\code{not}~A$.\
They are satisfied whenever there positive counter-parts are satisfied. 
But unlike with positive literals, such literals are not subject to minimization.
...
Sometimes it is useful to use double negation,
for example 

flies(X)     :- not not flies(X), bird(X).
not flies(X) :- penguin(X).

the second rule can also be written using a doubl negation.
