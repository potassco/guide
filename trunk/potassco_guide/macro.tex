% Logic programs

\newcommand{\alt}[1]{\ensuremath{\mathbf{#1}}}
\newcommand{\ground}[1]{\ensuremath{\mathit{ground}(#1)}}
\newcommand{\vars}[1]{\ensuremath{\mathit{var}(#1)}}
\newcommand{\head}[1]{\ensuremath{\mathit{head}(#1)}}
\newcommand{\pbody}[1]{\ensuremath{\mathit{body}^+(#1)}}
\newcommand{\nbody}[1]{\ensuremath{\mathit{body}^-(#1)}}
\newcommand{\binders}[2]{\ensuremath{\mathit{bind}(#1)}}
\newcommand{\atom}[1]{\ensuremath{\mathit{atom}(#1)}}

\newcommand{\reduct}[2]{\ensuremath{#1^{#2}}}

% Asprin
% \newcommand{\pf}[0]{\ensuremath{\mathit{preference}}} % {\ensuremath{p}}
% \newcommand{\name}[0]{\ensuremath{\mathit{name}}}
%\newcommand{\cardinality}{card}
%\newcommand{\lexico}{lex}

% Systems

\newcommand{\sysfont}[1]{\texttt{#1}}

\newcommand{\aspartame}[0]{\sysfont{aspartame}}
\newcommand{\assat}[0]{\sysfont{assat}}
\newcommand{\asprin}[0]{\sysfont{asprin}}
\newcommand{\clasp}[0]{\sysfont{clasp}}
\newcommand{\claspD}[0]{\sysfont{claspD}}
\newcommand{\hclasp}[0]{\sysfont{hclasp}}
\newcommand{\claspfolio}[0]{\sysfont{claspfolio}}
\newcommand{\claspre}[0]{\sysfont{claspre}}
\newcommand{\piclasp}[0]{\sysfont{piclasp}}
\newcommand{\smac}[0]{\sysfont{SMAC}}
\newcommand{\clingo}[0]{\sysfont{clingo}}
\newcommand{\clingcon}[0]{\sysfont{clingcon}}
\newcommand{\cmodels}[0]{\sysfont{cmodels}}
\newcommand{\dlv}[0]{\sysfont{dlv}}
\newcommand{\gnt}[0]{\sysfont{gnt}}
\newcommand{\gringo}[0]{\sysfont{gringo}}
\newcommand{\dlvcomplex}[0]{\sysfont{dlv-complex}}
\newcommand{\iclingo}[0]{\sysfont{iclingo}}
\newcommand{\oclingo}[0]{\sysfont{oclingo}}
\newcommand{\lparse}[0]{\sysfont{lparse}}
\newcommand{\nomorepp}[0]{\sysfont{nomore++}}
\newcommand{\smodels}[0]{\sysfont{smodels}}
\newcommand{\smodelscc}[0]{\sysfont{smodels}$_{\text{\sysfont{cc}}}$}
\newcommand{\lua}{\sysfont{lua}}
\newcommand{\python}{\sysfont{python}}

% Theorems

\theoremstyle{definition}
\newtheorem{texample}{Example}[section]
\newtheorem{tnote}{Remark}[section]

\newenvironment{note}{\begin{tnote}}{\hfill$\blacksquare$\end{tnote}}
\newenvironment{example}{\begin{texample}}{\hfill$\blacksquare$\end{texample}}

% More formatting

\lstset{numbers=left,numberblanklines=false,basicstyle=\ttfamily,xleftmargin=0pt}

% Note that the fbox below adds a little spacing to the left and right
\newlength{\marginlabelsize}
\newlength{\emptyfboxwidth}
\settowidth{\emptyfboxwidth}{\fbox{}}
\setlength{\marginlabelsize}{\marginparwidth}
\addtolength{\marginlabelsize}{-\emptyfboxwidth}

\newcommand{\marginlabel}[1]{\marginpar{\fbox{\parbox[t]{\marginlabelsize}{\footnotesize #1}}}}

\newcommand{\code}[1]{\texttt{#1}}
\newcommand{\codeit}[1]{\code{\textit{#1}}}
\newcommand{\const}[1]{\texttt{#1}}%{\ensuremath{\mathit{#1}}}
\newcommand{\pred}[1]{\texttt{#1}}%{\ensuremath{\mathsf{#1}}}
\newcommand{\var}[1]{\texttt{#1}}%{\ensuremath{\mathit{#1}}}

% \newcommand{\mus}[0]{\raisebox{-1.2pt}{\hspace{-0.4pt}\_\hspace{-0.7pt}\_}\hspace{-0.3pt}}%{\_\hspace{-2pt}\_\hspace{-2pt}\_}
\renewcommand{\dagger}{**}

% this macro is kinda ugly but the only way I found to workaround warnings
\def\attach{\textattachfile[color=1 0 0]}

% some language definitions

\lstdefinelanguage{GringoPython}%
  {morekeywords={script,and,break,class,continue,def,del,elif,else,%
      except,finally,for,from,global,if,import,in,is,lambda,not,%
      or,pass,print,raise,return,try,while},%
   sensitive=true,%
   morecomment=[l]\#,%
   morestring=[b]',%
   morestring=[b]",%
   morecomment=[s]{'''}{'''},%
   morecomment=[s]{"""}{"""}%
  }%

\lstdefinelanguage{GringoLua}%
  {morekeywords={end,script,function,for,if,local,require,print,and,or,not,else,return,then},%
   sensitive=true,%
   morecomment=[l]{--},%
   morecomment=[l]\#,%
   morestring=[b]',%
   morestring=[b]",%
   morecomment=[s]{--[[}{--]]}%
  }%

\lstset{literate={~}{{\raise.17ex\hbox{$\scriptstyle\sim$}}}1}

%%% Local Variables: 
%%% mode: latex
%%% TeX-master: "guide"
%%% End: 
