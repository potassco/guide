...
`\code{a(X)}', 
`\code{C::edge(X,Y,C)}', 
`\code{W,(X,Y)::\texttildelow a(W,X) \& b(Y)}', and 
`\code{X::name(p(X))}'. 
...
and $M_3\succ_{\code{p1}}M_2$ given that \code{\{a(2),a(3)\}$\subset$\{a(1),a(2),a(3)\}}, 
...
the three involved elements \code{s}, $X$ and $Y$ are translated into rules and facts.
...
replaces \code{p1} with \code{p2} in the previous rules, and adds:
\begin{lstlisting}[numbers=none]
...
For example, 
the formula \code{(\texttildelow a(X) | b(X)) \& c(X)} is translated into \code{\_and(c(X),\_or(\_neg(a(X)),b(X)))}. 
%
...
\begin{example}
\comment{JR: This is a ground preference specification, maybe we could add variables.}
...
%\begin{lstlisting}[numbers=none,escapechar=?]
%preference(s,(?$j$?,?$\boldsymbol{v}$?),?$i$?,for(?$\boldsymbol{t}_F$?),?$\boldsymbol{t}$?).
%\end{lstlisting}
...
\asprin\ 
provides a fact \code{holds($\boldsymbol{t}_F$)} 
for every term \code{for($\boldsymbol{t}_F$)} appearing in the translation of \code{s} 
such that $X$ satisfies the Boolean formula $F$ 
and, analogously, it provides the fact \code{holds'($\boldsymbol{t}_F$)} if $Y$ satisfies $F$.
...
\code{holds(a(1))}, \code{holds'(a(1))}, \code{holds'(a(2))} and \code{holds'(a(3))} are given.
...
\code{holds(a(1))}, \code{holds'(a(2))} and \code{holds'(a(3))} are provided.
\end{example}
...
%
It states that \code{better(P)} must hold whenever \code{P} is optimized. 
Given that this rule is shared by many preference programs, 
we include it in a program named simply \code{preference}.
...
Recall from the previous section that for each preference type 
we define a set of admissible preference statements.
This is defined in \asprin\ in the preference program using predicate \code{\_error/1}.
Namely, \code{\_error(X)} has to be true whenever the preference statement is not admissible.
In this case, \asprin\ stops and prints the error message \code{X}\footnote{
...
Function \code{@cat(*args)}, provided by \asprin, simply concatenates the terms that are passed as arguments.
\end{example}
...
(but not using \code{holds/1} or \code{holds'/1}).
