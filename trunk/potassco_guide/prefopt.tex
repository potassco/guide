...
in the weighted formula, write a variable (e.g., \code{V}) instead of the element (e.g., \code{a(X)}),  
...
A \emph{preference type} \code{t} is a function mapping a set of ground preference elements
\code{\{$e_1$;$\ldots$;$e_n$\}} to a preference relation.
...
For example, the preference type \code{subset} is defined as follows: 
%A set \code{$E=$\{$e_1$;$\ldots$;$e_n$\}} of ground preference elements
\begin{itemize}
\item
the domain of \code{subset}
consists of the  
%belongs to the domain of \code{subset} if every $e_i \in E$ is a Boolean formula, and
sets of ground preference elements \code{$E=$\{$e_1$;$\ldots$;$e_n$\}}
where every $e_i$ is a Boolean formula, and 
%\code{\{$e_1$;$\ldots$;$e_n$\}} of ground preference elements 
%belongs to the domain of \code{subset} if every $e_i \in E$ is a Boolean formula, and
\item
%A set \code{\{$e_1$;$\ldots$;$e_n$\}} of ground preference elements 
%is mapped \code{subset} to the strict partial order: 
\code{subset} maps every such set $E$ of preference elements to the preference relation defined as: 
\(
...
\end{lstlisting}
is \emph{admissible} if \code{\{${e}_1$;$\dots$;${e}_n$\}} belongs to the domain of \code{t}.
...
\end{lstlisting}
which is admissible because \code{a(1)$,\ldots,\,$a(5)} are Boolean formulas. 
Observe that \code{p1} becomes not admissible if we add to it the preference elements \code{1::a(1)} or \code{name(p2)}.

An admissible preference statement \code{p} declares a preference relation $\succ_{\code{p}}$ on the answer sets of the base program, 
that is the result of mapping the preference type of \code{p} on the set of its ground preference elements.
For example, \code{p1} declares the preference relation $\succ_{\code{p1}}$:
\begin{tabular}{ccl}
...
In Example \ref{asprin:example1}, with preference statement \code{p1},
