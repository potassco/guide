...
%             in the system path.}%
\marginlabel{\clingo\ Python module must be importable, and the related files \code{asprin} and \code{asprin.lib}
             ought to be located in some directory
             in the system path.}%
...
A \emph{weighted body} has form
\[\boldsymbol{t}\code{::}B\]
where $\boldsymbol{t}$ is a term tuple,
and $B$ is a either a body of Boolean formulas
\[F_1, \ldots, F_n\]
or a naming atom.
We may drop \code{::} and simply write $B$ whenever $\boldsymbol{t}$ is empty.
If $B$ is missing\footnote{Note that empty weighted bodies are not allowed},
...
%
Examples of weighted bodies are 
`\code{42::a}', 
`\code{a(X), b(X)}', 
...
If $B_1$, \ldots, $B_n$ are weighted bodies, then
\[
\code{\{}B_1\code{;}\dots\code{;}B_m\code{\}}
\]
is a set of weighted bodies.
...
\[\boldsymbol{B}_1\; >> \,\dots\, >> \;\boldsymbol{B}_m\code{ || }\boldsymbol{B}\code{ : }\!\!\!\! B\]
\comment{JR: Too many Bs?, maybe rename the weighted bodies?}
where each $\boldsymbol{B}_r$ and $\boldsymbol{B}$ is a set of weighted bodies, 
...
and `\code{||$\,F$}' and `\code{:$\,B$}' whenever $\boldsymbol{B}$ and/or $B$ are empty, respectively.
%
Intuitively, $r$ gives the rank of the respective set of weighted bodies.
This can be made subject to condition $\boldsymbol{B}$ by using the conditional `\code{||}'.
...
%
And similarly, \asprin\ adds facts  
...
\end{note}
\comment{JR: Move out of the note? }
...
\comment{JR: STOPPED HERE, I'll continue once we are happy with the previous part.}
...
\comment{JR: move Boolean formulas to bodies of Boolean formulas? Let's decide first if we stay with those names  }
...
\marginlabel{%
 To inspect the output, invoke:\\
  \code{\mbox{~}asprin \attach{examples/base.lp}{base.lp} \mbox{~~~~}\textbackslash \
  \mbox{~~~}\attach{examples/preference3.lp}{preference3.lp}\mbox{~~~}\textbackslash \ 
  \mbox{~~~}asprin.lib}}
%
...
\marginlabel{%
 To inspect the output, invoke:\\
  \code{\mbox{~}asprin \attach{examples/base.lp}{base.lp} \mbox{~~~~}\textbackslash \
  \mbox{~~~}\attach{examples/preference4.lp}{preference4.lp}\mbox{~~~}\textbackslash \ 
  \mbox{~~~}asprin.lib}}
%
...
\marginlabel{%
 To inspect the output, invoke:\\
  \code{\mbox{~}asprin \attach{examples/base.lp}{base.lp} \mbox{~~~~}\textbackslash \
  \mbox{~~~}\attach{examples/preference5.lp}{preference5.lp}\mbox{~~~}\textbackslash \ 
  \mbox{~~~}asprin.lib}}
%
