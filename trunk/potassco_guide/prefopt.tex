...
The format is detailed in \cite{brderosc15a}.\comment{T: Shouldn't this be here as well?}
...
For example, the type \code{subset},
...
Intuitively, \code{better(P)} is true if $H$ is preferred to $H'$ with respect to a preference statement $P$ of type \code{subset}. 
More formally, 
\code{better(P)} is true if there is one atom \code{X} of the preference statement that is false in $H$ and true in $H'$, 
and all the atoms \code{Y} of the preference statement that are false in $H'$ are also false in $H$.  
